...
> **`manual-control` branch (built off `preplanning`): fly the drone by keyboard to map the course.** Vision and the autonomous planner are OFF (`USE_TELEOP=True` in `main.py`, `USE_VISION=False`). `setup.py` swaps in `teleop.TeleopPlanner` (same `compute_target()` → `{'mode':'velocity','vel_ned','yaw',...}` contract — the controller is untouched) and starts a `teleop.KeyboardTeleop` background thread (the usual `create_*`/`get_thread_for_join` lifecycle). **Controls:** `W/S` forward/back, `A/D` strafe left/right, `Space`/`Ctrl` climb/descend, `Q/E` yaw left/right, **`B` captures the drone's current NED pose** as a waypoint and rewrites `captured_waypoints.json` (`CAPTURE_PATH` env) in the exact `mission.save_mission` schema — so a captured file can be dropped onto the `preplanning` branch as a `mission.json` and replayed. WASD is mapped along the **physical nose** `(cos yaw, −sin yaw)` (the inverse of `velocity_to_attitude`'s body loop) so motion matches the FPV camera; `Q/E` integrate a heading setpoint the controller turns toward (flip `teleop.TELEOP_YAW_SIGN` if reversed). The keyboard thread reads true key STATE via Win32 `GetAsyncKeyState` (held keys + modifiers like Ctrl work); a POSIX raw-tty fallback exists for offline use (no Ctrl there → descend on `C`). Speeds via `TELEOP_SPEED_MPS` / `TELEOP_VSPEED_MPS` / `TELEOP_YAWRATE_DPS`. Offline test: `python test_teleop.py`. Set `USE_TELEOP=0` to fall back to the square/mission flight. **CAUTION:** `DRY_RUN=False` here too — the drone arms and flies on startup (it hovers until you press a key).
