...
class RepositoryFrameTests(unittest.TestCase):
    def test_32_floor_markers_rejected_and_real_gate_detected(self):
        ...
        floor_markers = detector.detect(
            cv2.imread(str(frames / "f_13300.jpg"), cv2.IMREAD_COLOR)
        )
        self.assertFalse(floor_markers.found)

        real_gate = detector.detect(
            cv2.imread(str(frames / "f_14400.jpg"), cv2.IMREAD_COLOR)
        )
        self.assertTrue(real_gate.found)
        self.assertEqual(real_gate.method, "inner_contour")
        self.assertGreater(real_gate.confidence, 0.60)
        self.assertAlmostEqual(real_gate.center_x, 320.0, delta=12.0)
        self.assertAlmostEqual(real_gate.center_y, 180.0, delta=12.0)
